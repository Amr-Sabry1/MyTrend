...
export default function Home() {
  ...
  let [Typetv, setTypetv] = useState("popular");
  ...
  async function getTrending() {
    const res = await fetch(
      `https://api.themoviedb.org/3/tv/${Typetv}?api_key=7c9289e094fd00ffc1fa0413b31d4795`
    );
    const data = await res.json();
    settv(data.results);
    if (data.results == "") {
    ...
    <>
      <div className="container my-5 mt-4 py-5">
        <div className="position-relative">
          <img
            src="https://wallpapers.com/images/featured/game-of-thrones-92acb30ilmkjbmu9.jpg"
            className="mainimg movieMainImg"
            alt=""
          />
          <div className="captionmainImg moviecaptionmainImg">
            <h2>Welcome. Millions of tv to Discover.</h2>
            <h3>Explore now.</h3>
        ...
        <div className="row">
          <div className="col-md-3">
            <div class="input">
              ...
              <button class="value" onClick={() => shangeType("on_the_air")}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 32 32"
                  id="Line"
              ...
              <button class="value" onClick={() => shangeType("airing_today")}>
          ...
          <div className="col-md-9">
            <div className="row g-3">
              {tv?.slice(0, x).map((movie) => (
                <MainItem key={movie.id} data={movie} type={"tv"} />
              ))}
            </div>
            <div id="Hide" className="d-none m-3 text-center">
              <h3>No tvshows to Show ...</h3>
            </div>
