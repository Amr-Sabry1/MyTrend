...
export default function Home() {
  ...
  let [TypeMovie, setTypeMovie] = useState("popular");

  async function search(e) {
    const res = await fetch(
      `https://api.themoviedb.org/3/search/movie?api_key=7c9289e094fd00ffc1fa0413b31d4795&query=${e.target.value}&include_adult=false&language=en-US&page=1`
    );
  ...
  async function getTrending() {
    const res = await fetch(
      `https://api.themoviedb.org/3/movie/${TypeMovie}?api_key=7c9289e094fd00ffc1fa0413b31d4795`
    );
    const data = await res.json();
    setmovies(data.results);
    if (data.results == "") {
    ...
    <>
      <div className="container my-5 mt-4 py-5">
        <div className="position-relative">
          <img
            src="https://c4.wallpaperflare.com/wallpaper/155/746/467/movie-john-wick-chapter-3-parabellum-john-wick-keanu-reeves-hd-wallpaper-thumb.jpg"
            className="mainimg movieMainImg"
            alt=""
          />
          <div className="captionmainImg moviecaptionmainImg">
            <h2>Welcome. Millions of Movies to Discover.</h2>
            <h3>Explore now.</h3>
        ...
        <div className="row">
          <div className="col-md-3">
            <div class="input">
              <button class="value" onClick={() => shangeType("popular")}>
                <svg
                  data-name="Layer 2"
              ...
              <button class="value" onClick={() => shangeType("now_playing")}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 32 32"
                  id="Line"
              ...
              <button class="value" onClick={() => shangeType("upcoming")}>
              ...
              <button class="value" onClick={() => shangeType("top_rated")}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 32 32"
                  id="svg8"
          ...
          <div className="col-md-9">
            <div className="row  g-3">
              {movies?.slice(0, x).map((movie) => (
                <MainItem key={movie.id} data={movie} type={"movies"} />
              ))}
            </div>
